...
المزايا:
  • ⏱️ استفتاء كويز بعدّاد زمني حقيقي لكل سؤال.
  • ✅ تصحيح فوري + 💡 شرح لكل إجابة.
  • 🏆 لوحة صدارة لأفضل النتائج.
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        "👨‍💻 <b>بوت اختبارات مادة البرمجة المهيكلة (C++)</b>\n\n"
        "مرحباً بك في منصة الاختبارات الشاملة لنماذج امتحانات الميد <b>(A, B, C, D, E, F)</b>.\n"
        "تم إعداد وبرمجة هذا البوت بواسطة الطالب: <b>باسم عصيد كمر</b>.\n\n"
        "<b>مزايا الاختبار:</b>\n"
        "⏱️ عدّاد زمني حقيقي لكل سؤال.\n"
        "✅ تصحيح فوري مع إيضاح الحل الصحيح.\n"
        "🏆 تسجيل النقاط في لوحة الصدارة.\n\n"
        "اختر لغة الاختبار للبدء 👇"
    )
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📘 بدء الاختبار — عربي", callback_data="q|cpp|ar")],
        [InlineKeyboardButton("📗 Start Quiz — English", callback_data="q|cpp|en")],
    ])
...
def run_meta(mode):
    return (True, "البرمجة المهيكلة (عربي)" if mode == "ar" else "Structured Programming (English)")
...
async def on_start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    ranked, label = run_meta(mode)
    context.user_data.update(
        quiz=build_quiz(mode), idx=0, score=0, ranked=ranked, board_label=label,
    ...
    if u.username:
        who += f" (@{u.username})"
    await notify_admin(context, f"🔔 دخل الطالب {who} وبدأ اختبار: {label}.")
...
async def finish(context, chat_id) -> None:
    ...
    if pct == 100:
        ...
        note, emoji = "جيد، لكن الكود يحتاج لبعض المراجعة (Debugging).", "📖"
    else:
        note, emoji = "راجع الملزمة جيدًا وأعد تشغيل البرنامج (Run again).", "💪"
...
async def show_board(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not _is_admin(user.id):
        if update.callback_query:
            await update.callback_query.answer("هذه اللوحة خاصة بالمشرف فقط.", show_alert=True)
        else:
            await update.message.reply_text("هذه اللوحة خاصة بالمشرف فقط.")
        return
    board = context.bot_data.setdefault("board", load_board())
...
def main() -> None:
    ...
    print(f"✅ البوت يعمل (مادة البرمجة C++ فقط، ⏱️ {TIME_LIMIT}ث/سؤال)... Ctrl+C للإيقاف.")
