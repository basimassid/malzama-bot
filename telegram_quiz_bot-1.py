...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "كل سؤال على شكل استفتاء مع تصحيحٍ فوري وشرحٍ للإجابة، وعدّاد ⏱️ ظاهر. "
        "لا يظهر السؤال التالي ولا الإجابة الصحيحة إلا بعد اختيارك.\n\nاختر المادة 👇"
    )
...
async def on_start_quiz(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        f"✅ بدأنا: <b>{label}</b> ({n} سؤالًا)\nأجب عن كل سؤال للانتقال للتالي — بالتوفيق! 🍀",
        parse_mode=ParseMode.HTML)
    # إشعار المدير عند كل دخول/بدء اختبار
...
async def send_question(context, chat_id) -> None:
    ...
    # لا open_period: يبقى السؤال مفتوحًا، ولا تظهر الإجابة إلا عند الاختيار
    ...
    ud["cur_idx"] = idx
    ud["cur_done"] = False
    # عدّاد نصّي ظاهر (لا يُنهي السؤال ولا ينتقل تلقائيًّا)
...
async def countdown(context, chat_id, idx, msg_id) -> None:
    """عدّاد ظاهر يُحدَّث كل ٥ ثوانٍ؛ لا ينقل السؤال ولا يكشف الإجابة."""
    try:
